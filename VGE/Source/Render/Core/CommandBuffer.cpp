...
{
	BindBuffer(lightingState.LightBuffer.GetBuffer(), lightingState.LightBuffer.GetOffset(), lightingState.LightBuffer.GetSize(), set, binding, 0);

	SetSpecializationConstant(0, ToU32(lightingState.DirectionalLights.size()));
	SetSpecializationConstant(1, ToU32(lightingState.PointLights.size()));
	SetSpecializationConstant(2, ToU32(lightingState.SpotLights.size()));
}
...
void vge::CommandBuffer::FlushPipelineState(VkPipelineBindPoint pipelineBindPoint)
{
	...
	if (pipelineBindPoint == VK_PIPELINE_BIND_POINT_GRAPHICS)
	{
		...
		auto& pipeline = _Device->GetResourceCache().RequestGraphicsPipeline(_PipelineState);
		vkCmdBindPipeline(GetHandle(), pipelineBindPoint, pipeline.GetHandle());
	}
	else if (pipelineBindPoint == VK_PIPELINE_BIND_POINT_COMPUTE)
	{
		auto& pipeline = _Device->GetResourceCache().RequestComputePipeline(_PipelineState);
		vkCmdBindPipeline(GetHandle(), pipelineBindPoint, pipeline.GetHandle());
	}
	else
	{
