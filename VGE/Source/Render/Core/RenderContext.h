...
public:
	...
	inline void Submit(CommandBuffer& command_buffer) { Submit({ &command_buffer }); }
	...
	void Prepare(size_t threadCount = 1, RenderTarget::CreateFunc createRenderTargetFunc = RenderTarget::DefaultCreateFunction);
	...
	CommandBuffer& Begin(CommandBuffer::ResetMode resetMode = CommandBuffer::ResetMode::ResetPool); // prepares the next available frame for rendering

	void Submit(const std::vector<CommandBuffer*>& commandBuffers);
	void Submit(const Queue& queue, const std::vector<CommandBuffer*>& commandBuffers);
	VkSemaphore Submit(const Queue& queue, const std::vector<CommandBuffer*>& commandBuffers, VkSemaphore waitSemaphore, VkPipelineStageFlags waitPipelineStage);
