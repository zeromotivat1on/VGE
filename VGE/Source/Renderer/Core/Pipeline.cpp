...
vge::Pipeline::Pipeline(Pipeline&& other) 
	: device(other.device), handle(other.handle), state(other.state)
{
	...
	: Pipeline(device)
{
	...
	VK_ENSURE(vkCreateComputePipelines(device.GetHandle(), pipelineCache, 1, &createInfo, nullptr, &handle));
	...
	: Pipeline(device)
{
	...
	VK_ENSURE(vkCreateGraphicsPipelines(device.GetHandle(), pipelineCache, 1, &createInfo, nullptr, &handle));
